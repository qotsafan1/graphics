...
void PoolScene::set_light_and_camera(ShaderProgramDraw& shader_prog, bool is_reflected)
{
    ...
    shader_prog.set_projection_matrix(perspective_Mat4x4f(55, float(window_width)/window_height, 0.01f, 100.0f));
...
void update_flocking()
{
    ...
    if (!fbo1)
    {
        ...
        for (uint i = 0; i < TEX_SIZE; ++i)
            for (uint j = 0; j < TEX_SIZE; ++j)
            {
