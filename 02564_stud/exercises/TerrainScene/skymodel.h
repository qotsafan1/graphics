...
{
public:
  PreethamSunSky(unsigned int day = 200, float time = 8.0f, float latitude = 45.0f, float turbidity = 2.0f, float overcast = 0.0f);

  DirectionalLight get_directional_light(float scale);

  void setTime(unsigned int day, float time) { _day = day; _time = time; CGLA::Vec2f coord = sun_position(_day, _time, _latitude); setSunTheta(coord[0]); setSunPhi(coord[1]); }
  void setLatitude(float latitude)           { _latitude = latitude; CGLA::Vec2f coord = sun_position(_day, _time, _latitude); setSunTheta(coord[0]); setSunPhi(coord[1]); }
  ...
  // Query the sun color at current sun position and air turbidity ( W/m^2/sr )
  CGLA::Vec3f sunColor();

  // Query the sky color in a given direction ( W/m^2 )
  ...
  void precomputeTexture(GLuint &cubetex, int size);  

private:
  CGLA::Vec3f calculateSunColor();
  void spencer(unsigned int day, double& earth_sun_dist, double& declination);
  CGLA::Vec2f solar_elevation(double time, double latitude, double declination);
  CGLA::Vec2f sun_position(unsigned int day, double time, double latitude);
